...
(function(lib) {
  ...
  lib.transform = function(selector){
    lib.load(selector, function(element, css) {
      ...
      for(var entry in css) {
        if( ! lib.send(entry, css[entry])) {
          replacement += lib.standard(entry, css[entry]);
        }
      }

      // $('head').append($('<style type="text/css">' + replacement + '</style>'));
      // $('body').append($('<pre>' + replacement + '</pre>'));

      if($('head').find('style#temp_hack').length == 0) {
        $('head').append($('<style id="temp_hack" type="text/css">' + replacement + '</style>'));
      }
    });
  ...
  lib.standard = function(selector, declaration) {
    ...
    //   * be sure the order is maintained.
    //   * CSS builder?
